import React, { useState, useEffect, memo, useMemo, useDeferredValue, useCallback, useRef, useLayoutEffect } from 'react';
...
import {
    ...
    IconVolumeX, IconVolume1, IconVolume2
} from './Icons';
import {
...
const ShelfItem = memo(({ item, themeDef, engravingDef, isSelectionMode, selectedMangas, inspectingMangaId, deletingMangas, handleBookPointerEnter, toggleMangaSelection, setInspectingManga, handleReorderManga, isTutorialActive }) => {
    ...
    return (
        <div key={m.id} className="flex-none flex items-end relative group/book hover:z-[100]" style={{ height: 'var(--row-total)', paddingTop: 'var(--row-pt)', paddingBottom: 'var(--board-h)' }}>
            <div id={isTutorialActive && item.isFirst ? 'tutorial-first-book' : `book-${m.id}`} draggable={!isSelectionMode} onDragStart={(e) => { if (isSelectionMode) { e.preventDefault(); return; } e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', m.id); e.currentTarget.style.opacity = '0.4'; }} onDragEnd={(e) => { e.currentTarget.style.opacity = ''; }} onDragOver={(e) => { if (isSelectionMode) return; e.preventDefault(); e.dataTransfer.dropEffect = 'move'; }} onDrop={(e) => { if (isSelectionMode) return; e.preventDefault(); e.stopPropagation(); const draggedId = e.dataTransfer.getData('text/plain'); if (draggedId && draggedId !== m.id && handleReorderManga) { handleReorderManga(draggedId, m.id); } }} onPointerEnter={handleBookPointerEnter} onClick={(e) => { if (isSelectionMode) { e.preventDefault(); toggleMangaSelection(m.id); return; } setInspectingManga(m, e); }} className={`manga-cover-image relative flex-none cursor-pointer z-10 rounded-[2px] ${isDeleting ? 'scale-0 opacity-0 pointer-events-none' : isInspected ? 'opacity-0 pointer-events-none' : isSelected ? 'ring-2 ring-theme-500 scale-95 opacity-80' : 'hover:-translate-y-5 hover:shadow-[0_20px_30px_-8px_rgba(0,0,0,0.9),0_0_20px_rgba(var(--theme-rgb),0.15)]'}`} style={{ backgroundColor: '#0f172a', height: '95%', width: `calc(var(--row-book-h) * 0.95 * (${bd} / ${bh}))`, boxShadow: '0 8px 10px -4px rgba(0,0,0,0.8)', transition: isDeleting ? 'transform 0.4s cubic-bezier(0.5, 0, 1, 0.5), opacity 0.4s ease-out' : 'transform 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), box-shadow 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.35s ease', willChange: 'transform' }}>
                <div className="peek-popup absolute top-2 left-[calc(100%+12px)] bg-slate-900/95 backdrop-blur-xl rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_20px_rgba(var(--theme-rgb),0.3)] opacity-0 group-hover/book:opacity-100 group-hover/book:translate-y-0 pointer-events-none z-[200] overflow-hidden border border-white/10 flex flex-col w-[140px] sm:w-[180px] origin-top-left translate-y-2" style={{ transition: 'opacity 0.2s ease, transform 0.28s cubic-bezier(0.34, 1.4, 0.64, 1)' }}>
                    <div className="relative w-full bg-black border-b border-white/10" style={{ aspectRatio: `${bw} / ${bh}` }}><StackThumbnail file={mainCover} contain={true} /></div>
                ...
                <div className="absolute inset-0 flex items-center justify-center overflow-hidden rounded-[2px] bg-[#0f172a]">
                    {isDouble && wrapCoverUrl ? (<div className="w-full h-full opacity-100 transition-opacity" style={{ backgroundImage: `url(${wrapCoverUrl})`, backgroundSize: bgSizeSpine, backgroundPosition: 'center center', backgroundRepeat: 'no-repeat' }}></div>) : (<>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <span className={`${themeDef.text} group-hover/book:text-white font-black uppercase tracking-widest overflow-hidden whitespace-nowrap [text-overflow:clip] text-center leading-none transition-colors absolute`} style={{ transform: 'rotate(90deg)', textShadow: '0px 2px 5px rgba(0,0,0,1), 0px 0px 2px rgba(0,0,0,0.8)', fontSize: 'clamp(9px, 1.6vh, 13px)', width: 'max-content', padding: '0 10px' }}>{m.title}</span>
                        </div>
                    </>)}
                    {m.bookmark != null && !m.isFinished && (<div className="absolute top-0 left-1/2 -translate-x-1/2 w-[35%] max-w-[16px] min-w-[6px] h-[22%] bg-theme-500 shadow-[0_4px_10px_rgba(0,0,0,0.9)] z-40 border-x border-b border-white/40 animate-bookmark-in" style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 50% 80%, 0 100%)', backgroundImage: 'linear-gradient(to bottom, rgba(255,255,255,0.4), rgba(0,0,0,0.2))' }}></div>)}
                    {m.isFinished && (<div className="absolute bottom-3 left-1/2 -translate-x-1/2 w-[25%] max-w-[12px] min-w-[6px] aspect-square rounded-full bg-amber-400 shadow-[0_0_10px_rgba(251,191,36,1)] border border-white/80 z-40"></div>)}
                </div>
                {isSelectionMode && (<div className={`absolute inset-0 z-30 flex items-center justify-center transition-all rounded-[2px] ${isSelected ? 'bg-theme-900/60 backdrop-blur-[2px]' : 'bg-black/40 group-hover/book:bg-black/20'}`}><div className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-theme-500 bg-theme-500 text-white shadow-[0_0_15px_rgba(var(--theme-rgb),0.8)]' : 'border-white/50 text-transparent'}`}><IconCheck width="12" height="12" strokeWidth="3" /></div></div>)}
...
const Row = memo(({ index, style, data }) => {
    const { virtualRows, pillarWidth, ...rest } = data;
    const row = virtualRows[index];
    const themeDef = SHELF_THEMES[rest.shelfTheme] || SHELF_THEMES.mahogany;
    const engravingDef = SHELF_ENGRAVINGS[rest.shelfEngraving] || SHELF_ENGRAVINGS.none;

    return (
        <div style={style}>
            <div className="relative h-full w-full">
                <div className="absolute top-0 left-0 w-full flex flex-col pointer-events-none z-0">
                    <div className="w-full flex-none flex flex-col justify-end transition-all duration-500" style={{ height: 'var(--row-total)' }}>
                        ...
                        <div className="w-full shadow-[0_20px_40px_rgba(0,0,0,1)] relative z-20 border-t border-white/5" style={{ height: 'var(--board-h)', ...themeDef.board }}>
                            ...
                            {engravingDef.style.backgroundImage && (<div className="absolute inset-0 pointer-events-none" style={engravingDef.style}></div>)}
                        </div>
                    </div>
                </div>
                <div className="relative z-30 flex flex-wrap content-start items-start justify-start w-full" style={{ paddingLeft: 'var(--pillar-w)', paddingRight: 'var(--pillar-w)' }}>
                    {row.items.map((item) => (
                        <ShelfItem key={item.key || item.data.id} item={item} themeDef={themeDef} engravingDef={engravingDef} {...rest} />
                    ))}
                </div>
            </div>
        </div>
...
const HubView = memo(({
    ...
    setAppTheme, shelfTheme, setShelfTheme, handleExport, handleImport, setPurgeConfirm, shelfEngraving, setShelfEngraving, animationSpeed, setAnimationSpeed, pageAnimationsEnabled, setPageAnimationsEnabled, soundVolume, setSoundVolume, isTutorialActive,
    ...
    const handleScroll = useCallback(({ scrollOffset }) => {
        ...
        scrollDebounceRef.current = setTimeout(() => {
            try {
                sessionStorage.setItem('mangaHubScrollPosition', String(scrollOffset));
            } catch (e) {
    ...
    const fuse = useMemo(() => new Fuse(normalizedMangas, {
    ...
    const searchResults = useMemo(() => {
        ...
        return fuse.search(deferredSearch).map(result => result.item).slice(0, 50);
    }, [fuse, deferredSearch]);
    ...
    }, [normalizedMangas, activeTags, showBookmarksOnly, sortOrder]);
    ...
    useLayoutEffect(() => {
        ...
        const observer = new ResizeObserver(() => {
            ...
            const itemsPerRow = Math.max(1, Math.floor((containerWidth - 2 * safePillarW) / (bookW + 8))); // +8 for some margin
    ...
    const virtualRows = useMemo(() => {
        if (layout.itemsPerRow <= 0 || libraryStructure.flattened.length === 0) return [];

        const rows = [];
        let i = 0;
        while (i < libraryStructure.flattened.length) {
            rows.push({ items: libraryStructure.flattened.slice(i, i + layout.itemsPerRow) });
        ...
        >
                                ...
                                <>
                                    ...
                                    <div className="absolute top-full right-0 mt-2 w-56 bg-slate-900/95 border border-theme-500/40 rounded-2xl shadow-[0_15px_40px_rgba(var(--theme-rgb),0.6)] z-[120] p-5 flex flex-col gap-5 animate-in origin-top-right">
                                        ...
                                        <div className="space-y-3">
                                            <span className="text-[10px] text-theme-400 font-black uppercase tracking-widest block opacity-70 text-center">Animation des Pages</span>
                                            <div className="grid grid-cols-2 gap-2">
                                                <button onClick={() => setPageAnimationsEnabled(true)} className={`py-1.5 rounded text-[10px] font-black transition-colors border ${pageAnimationsEnabled ? 'bg-theme-500 text-white border-theme-400 shadow-[0_0_10px_rgba(var(--theme-rgb),0.6)]' : 'bg-theme-950/50 text-theme-400 border-theme-800/50 hover:bg-theme-800/50'}`}>
                                                    Activées
                                                </button>
                                                <button onClick={() => setPageAnimationsEnabled(false)} className={`py-1.5 rounded text-[10px] font-black transition-colors border ${!pageAnimationsEnabled ? 'bg-theme-500 text-white border-theme-400 shadow-[0_0_10px_rgba(var(--theme-rgb),0.6)]' : 'bg-theme-950/50 text-theme-400 border-theme-800/50 hover:bg-theme-800/50'}`}>
                                                    Désactivées
                                                </button>
                                            </div>
                                            <div className={`grid grid-cols-3 gap-2 transition-opacity ${!pageAnimationsEnabled ? 'opacity-40 pointer-events-none' : ''}`}>
                                                <button onClick={() => setAnimationSpeed(1.5)} className={`py-1.5 rounded text-[10px] font-black transition-colors border ${animationSpeed === 1.5 ? 'bg-theme-500 text-white border-theme-400 shadow-[0_0_10px_rgba(var(--theme-rgb),0.6)]' : 'bg-theme-950/50 text-theme-400 border-theme-800/50 hover:bg-theme-800/50'}`}>
                                                    Lente
                                                </button>
                                                <button onClick={() => setAnimationSpeed(1.0)} className={`py-1.5 rounded text-[10px] font-black transition-colors border ${animationSpeed === 1.0 ? 'bg-theme-500 text-white border-theme-400 shadow-[0_0_10px_rgba(var(--theme-rgb),0.6)]' : 'bg-theme-950/50 text-theme-400 border-theme-800/50 hover:bg-theme-800/50'}`}>
                                                    Normale
                                                </button>
                                                <button onClick={() => setAnimationSpeed(0.6)} className={`py-1.5 rounded text-[10px] font-black transition-colors border ${animationSpeed === 0.6 ? 'bg-theme-500 text-white border-theme-400 shadow-[0_0_10px_rgba(var(--theme-rgb),0.6)]' : 'bg-theme-950/50 text-theme-400 border-theme-800/50 hover:bg-theme-800/50'}`}>
                                                    Rapide
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            <span className="text-[10px] text-theme-400 font-black uppercase tracking-widest block opacity-70 text-center">Volume des Sons</span>
                                            <div className="flex items-center gap-3">
                                                <button onClick={() => setSoundVolume(soundVolume > 0 ? 0 : 0.4)} className="text-theme-400 hover:text-white transition-colors flex-none">
                                                    {soundVolume === 0 ? <IconVolumeX width="20" height="20" /> : soundVolume < 0.5 ? <IconVolume1 width="20" height="20" /> : <IconVolume2 width="20" height="20" />}
                                                </button>
                                                <input
                                                    type="range"
                                                    min="0"
            ...
            {search.length > 1 ? (
                ...
                <main className="flex-1 flex flex-col overflow-hidden relative bg-black" style={{ paddingLeft: 'max(0px, env(safe-area-inset-left))', paddingRight: 'max(0px, env(safe-area-inset-right))' }}>
                    ...
                    <div ref={mainContainerRef} className="relative flex-1 w-full min-h-0 z-10 bg-[#050810]" style={{
