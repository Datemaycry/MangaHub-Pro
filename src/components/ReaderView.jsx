...
const SpreadDisplay = memo(({ spread, manga, isAnimating = false, slideDir = "next", isLandscape = true, hideAmbilight = false, singleRef, leftRef, rightRef }) => {
    ...
    if (spread.center) {
        ...
        let singleAnimClass = '';
        if (isAnimating && !isLandscape) {
            if (isRTL) singleAnimClass = slideDir === 'next' ? 'animate-enter-next-rtl' : 'animate-enter-prev-rtl';
            else singleAnimClass = slideDir === 'next' ? 'animate-enter-next-ltr' : 'animate-enter-prev-ltr';
        }
        if (isLandscape) {
            return (
                <div ref={singleRef} className="flex w-full h-full perspective-[3000px] transform-style-[preserve-3d]">
                    ...
                    <div ref={leftRef} className={`w-1/2 h-full relative overflow-hidden gpu-accelerated ${animLeftClass}`} style={{ willChange: 'transform, filter' }}>
                        <div className="absolute top-0 left-0 w-[100vw] h-full flex justify-center items-center">
                            <ReaderImage file={spread.center} data-nav-zone="true" style={{ pointerEvents: 'auto' }} />
                        </div>
                    </div>
                    <div ref={rightRef} className={`w-1/2 h-full relative overflow-hidden gpu-accelerated ${animRightClass}`} style={{ willChange: 'transform, filter' }}>
                        <div className="absolute top-0 right-0 w-[100vw] h-full flex justify-center items-center">
                            <ReaderImage file={spread.center} data-nav-zone="true" style={{ pointerEvents: 'auto' }} />
                        </div>
                    </div>
                </div>
            ...
            return (
                <div className="w-full h-full flex justify-center items-center perspective-[3000px] transform-style-[preserve-3d]">
                    {!hideAmbilight && <ReaderImage file={spread.center} isBackground={true} />}
                    <div ref={singleRef} className={`gpu-accelerated ${singleAnimClass}`}
                        style={{
                            height: '100%', width: 'auto', display: 'inline-flex', alignItems: 'center',
                            willChange: 'transform, filter', transformStyle: 'preserve-3d'
                        }}>
                        <ReaderImage file={spread.center} data-nav-zone="true" style={{ width: 'auto', height: '100%', objectFit: 'contain', pointerEvents: 'auto' }} />
                    </div>
                </div>
            );
        }
        ...
        <div className="double-layout relative">
            <div className="absolute inset-0 flex">
                <div className="w-1/2 h-full overflow-hidden relative">
                    {!hideAmbilight && spread.left && <ReaderImage file={spread.left} isBackground={true} />}
                </div>
                <div className="w-1/2 h-full overflow-hidden relative">
                    {!hideAmbilight && spread.right && <ReaderImage file={spread.right} isBackground={true} />}
                </div>
            </div>
            <div ref={leftRef} className={`page-wrapper-left gpu-accelerated ${animLeftClass}`} style={{ willChange: 'transform, filter' }}>
                {spread.left ? <ReaderImage file={spread.left} className="img-left" data-nav-zone="true" style={{ pointerEvents: 'auto' }} /> : <div className="w-full h-full bg-black"></div>}
            </div>
            <div ref={rightRef} className={`page-wrapper-right gpu-accelerated ${animRightClass}`} style={{ willChange: 'transform, filter' }}>
                {spread.right ? <ReaderImage file={spread.right} className="img-right" data-nav-zone="true" style={{ pointerEvents: 'auto' }} /> : <div className="w-full h-full bg-black"></div>}
            </div>
        </div>
    );
});
...
const ReaderView = memo(({
    ...
    toggleFullscreen, isFullscreen, toggleBookmark, currentManga, cursor, handleSetCursor, edgeGlow, currentPages,
    ...
    toggleDisplayMode, showSpine, toggleSpine, isJumping
}) => {
    ...
    const dragData = useRef({ startX: 0, startY: 0, diffX: 0, diffY: 0, progress: 0 });
    const activeRefs = useRef({ single: null, left: null, right: null });
    const dragRaf = useRef(null);
    const isPointerDown = useRef(false);

    // REFS DE NAVIGATION (pour les zones de clic)
    const setSingleRef = React.useCallback(el => { if (el) activeRefs.current.single = el; }, []);
    const setLeftRef = React.useCallback(el => { if (el) activeRefs.current.left = el; }, []);
    const setRightRef = React.useCallback(el => { if (el) activeRefs.current.right = el; }, []);
    ...
    const handlePointerDown = (e) => {
        ...
        dragData.current = { startX: e.clientX, startY: e.clientY, diffX: 0, diffY: 0, progress: 0 };
    ...
    const handlePointerMove = (e) => {
        ...
        const clientX = e.clientX; const clientY = e.clientY; const isRTL = currentManga?.direction === 'rtl';
        if (dragRaf.current) cancelAnimationFrame(dragRaf.current);
        dragRaf.current = requestAnimationFrame(() => {
            const diffX = clientX - dragData.current.startX;
            dragData.current.diffY = clientY - dragData.current.startY;
            let dir = null;
    ...
    const handlePointerUp = (e) => {
        if (dragRaf.current) cancelAnimationFrame(dragRaf.current);
        if (!isPointerDown.current || !allSpreads) return;
        isPointerDown.current = false;
        if (e.target.hasPointerCapture?.(e.pointerId)) e.target.releasePointerCapture(e.pointerId);

        const { diffX, diffY, progress, startX } = dragData.current;
        const { targetCursor, dir } = dragState;

        // Swipe vers le bas → ferme le lecteur
    ...
    useEffect(() => {
        if (!allSpreads?.length) return;
        const next = allSpreads[cursor + 1];
        if (!next) return;
        [next.center, next.left, next.right].forEach(file => {
            if (!file) return;
            const url = getCachedUrl(file);
    ...
    // Raccourci clavier n/N (mode nuit) — les autres raccourcis sont gérés par App.jsx
    useEffect(() => {
        const onKey = (e) => {
            if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return;
            if (e.key === 'n' || e.key === 'N') { setIsNightMode(prev => !prev); }
        };
        ...
        >
            <div className={`reader-ui-top ${zenMode ? 'opacity-0 pointer-events-none -translate-y-full' : 'opacity-100 translate-y-0'} flex items-center justify-between w-full px-4 md:px-8 mx-auto`}>
                ...
                <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
                    <div className="px-5 py-2 bg-black/60 backdrop-blur-md rounded-full border border-theme-600/30 text-[10px] sm:text-xs font-black tracking-widest text-white/80 pointer-events-none shadow-[0_0_15px_rgba(0,0,0,0.5)]">
                        {currentTime}
                    </div>
                    {currentManga?.chapters?.length > 0 && !isJumping && (() => {
                        const spread = allSpreads[cursor];
                        const validIndices = spread?.pageIndices?.filter(i => i >= 0) || [];
                        if (!validIndices.length) return null;
                        const pageIdx = Math.min(...validIndices);
                        const curCh = [...currentManga.chapters].sort((a, b) => b.startIndex - a.startIndex).find(c => pageIdx >= c.startIndex);
                        if (!curCh) return null;
                        return (
                            <div className="px-4 py-1 bg-theme-600/40 backdrop-blur-md rounded-lg border border-theme-400/50 text-[9px] font-black uppercase tracking-[0.15em] text-theme-100 pointer-events-none shadow-[0_0_15px_rgba(var(--theme-rgb),0.4)] animate-slide-down">
                                🔖 {curCh.name}
                            </div>
                        );
                    })()}
                ...
                <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <button onClick={toggleBookmark} onPointerDown={(e) => e.stopPropagation()} onPointerUp={(e) => e.stopPropagation()} className={`bg-black/80 backdrop-blur-md p-3 sm:p-4 rounded-xl sm:rounded-2xl active:scale-95 transition flex items-center justify-center border shadow-[0_0_15px_rgba(var(--theme-rgb),0.3)] hover:shadow-[0_0_25px_rgba(var(--theme-rgb),0.6)] ${currentManga?.bookmark === cursor ? 'text-theme-300 border-theme-400 bg-theme-900/40' : 'text-theme-400 border-theme-600/40 hover:text-theme-300 hover:bg-theme-900/30'}`} title={currentManga?.bookmark === cursor ? "Retirer le marque-page" : "Placer un marque-page ici"}>
            ...
            <div id="tutorial-reader-nav" className="w-full h-full flex justify-center items-center overflow-hidden relative" style={{ touchAction: 'none' }}
                ...
                onWheel={handleWheel}
            >
                <div className="absolute inset-0 z-10 flex justify-center items-center gpu-accelerated">
                    {(!allSpreads || allSpreads.length === 0 || isJumping) ? (
                        <div className="flex flex-col items-center gap-4 animate-pulse">
                            <div className="w-12 h-12 border-4 border-theme-400/20 border-t-theme-400 rounded-full animate-spin"></div>
                            <span className="text-theme-400 font-black uppercase tracking-widest text-[10px]">Chargement...</span>
                        </div>
                    ) : (
                        <SpreadDisplay
                            spread={allSpreads[cursor]}
                            manga={currentManga}
